...
class SingleStageTCN(nn.Module):
    ...
    def __init__(self, in_channel, n_features, n_classes, n_layers):  # 第一阶 2048 - 64
        super().__init__()
        self.conv_in = nn.Conv1d(in_channel, n_features, 1)
        layers = [
            DilatedResidualLayer(2**i, n_features, n_features) for i in range(n_layers)]      # 实现不同层数的不用的空洞卷积
        self.layers = nn.ModuleList(layers)
        self.conv_out = nn.Conv1d(n_features, n_classes, 1)

    def forward(self, x):  # shape
        out = self.conv_in(x)
        for layer in self.layers: # dilation
            out = layer(out)
        out = self.conv_out(out)
        return out                         # 每一阶的输出


class DilatedResidualLayer(nn.Module):                                           # 一层的空洞卷积的实现
    # Originally written by yabufarha
    # https://github.com/yabufarha/ms-tcn/blob/master/model.py
    ...
    def __init__(self, dilation, in_channel, out_channels):  
        ...
        self.conv_dilated = nn.Conv1d(
            in_channel, out_channels, 3, padding=dilation, dilation=dilation)    # 设置一维卷积的参数 博客 https://blog.csdn.net/sunny_xsc1994/article/details/82969867
        self.conv_in = nn.Conv1d(out_channels, out_channels, 1)                  # 1x1卷积的作用 https://zhuanlan.zhihu.com/p/37910136
        self.dropout = nn.Dropout()

    def forward(self, x):
        out = F.relu(self.conv_dilated(x))                                       # 
        out = self.conv_in(out)                                                  # https://zhuanlan.zhihu.com/p/37910136
...
class MultiStageTCN(nn.Module):
    ...
    def __init__(self, in_channel, n_classes, stages,
        ...
        if stages[0] == 'dilated':
            self.stage1 = SingleStageTCN(
                in_channel, n_features, n_classes, dilated_n_layers)       # 单独设置第一阶的参数
        elif stages[0] == 'ed':
            ...
            for stage in stages[1:]:
                if stage == 'dilated':
                    self.stages.append(SingleStageTCN(
                        n_classes, n_features, n_classes, dilated_n_layers))  # 剩下的每一阶段的参数都是一样的 
                elif stage == 'ed':
                    self.stages.append( 
    ...
    def forward(self, x):
        if self.training:
            # for training
            outputs = []
            out = self.stage1(x)       # 1 11 634 n c t     
            outputs.append(out)
